...
class SinglePeptideDataModule(BaseDataModule):
    def __init__(
        ...
        self.repo_name = (
                self.hparams.repo_id.split("/")[-1]
                if self.hparams.repo_id is not None
                else Path(self.hparams.data_dir).name)
    ...
    if self.hparams.repo_id is None:
        print(f"[INFO] Skipping Hugging Face download. Using local data from {self.hparams.data_dir}")
        return  # <-- skip snapshot_download

    # Otherwise, do the normal HF download
    local_dir = snapshot_download(
        repo_id=self.hparams.repo_id,
        local_dir=self.hparams.data_dir,
        repo_type="dataset",
        ignore_patterns=["*.pdb"],
    )
    ...
    def prepare_data(self):
        ...
        Changed this function from single_peptide_datamodule.py to disable the download huggingface logic.
        """
        local_path = f"{self.hparams.data_dir}/{self.repo_name}"
        if not os.path.exists(local_path):
            logging.warning(f"Local dataset not found at {local_path}.")
        else:
            logging.info(f"Using existing local dataset at {local_path}")
